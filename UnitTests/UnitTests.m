...
extern NSString *const URLReservedChars;
...
- (void) testShouldAppendSingleWhenURLContainsJustQueryBegin {
    XCTAssertEqualObjects([URL(@"http://www.foo.com/path?")
                           URLByAppendingQueryValue:@"value" forKey:@"key"].absoluteString,
                          @"http://www.foo.com/path?key=value",
                          @"Did not create correctly formatted URL");
}
...
- (void) testShouldAppendSingleWhenURLContainsEmptyQueryAndFragment {
    XCTAssertEqualObjects([URL(@"http://www.foo.com/path?#")
                           URLByAppendingQueryValue:@"value" forKey:@"key"].absoluteString,
                          @"http://www.foo.com/path?key=value#",
                          @"Did not create correctly formatted URL");
...
- (void) testShouldEncodeValuesContainingReservedCharacters {
    NSDictionary *dict = @{@"q": @"gin & tonic", @"other": URLReservedChars};
...
- (void) testShouldEncodeKeysContainingReservedCharacters {
    NSDictionary *dict = @{ URLReservedChars: @YES};
